...
if __name__ == "__main__":
  ...
  logistic_regression_unigram_with_positional = LogisticRegressionModel(
      use_positional_features=True)

  # cross validation
  for fold, data in dataset.items():
    print(
        f"Fold: {fold} Length of Train: {len(data['train'])} Length of Test: {len(data['test'])}")
    logistic_regression_unigram.fit(data['train'])
    logistic_regression_bigram.fit(data['train'])
    logistic_regression_unigram_with_positional.fit(data['train'])
    ...
    print(
        f"Fold: {fold} - Unigram Logistic Regression With Positional Features")
    print(logistic_regression_unigram_with_positional.predict(data['test']))
